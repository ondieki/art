...
if __name__ == "__main__":
    ...
    if os.path.exists(SWN_FILENAME):
        ...
        for senti_synset in swn.all_senti_synsets():
            if senti_synset.pos_score > 0.6 and senti_synset.pos_score < 0.8:
                token = senti_synset.synset.name.split('.')
                word = token[0]
                word = word.split('_')
                word = ' '.join(word)
                pos.write(word)
                pos.write('\n')
            elif senti_synset.neg_score > 0.6 and senti_synset.neg_score < 0.8:
                token = senti_synset.synset.name.split('.')
                word = token[0]
                word = word.split('_')
                word = ' '.join(word)
